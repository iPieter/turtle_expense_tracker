class DateFormatter {
  ...
  static String prettyPrint(DateTime date, [String language = "nl"]) {
    ...
    var diff =
        (now.millisecondsSinceEpoch - date.millisecondsSinceEpoch) / 1000.0;
    ...
    if (diff < 59.0) {
      ...
      //Hours
      if (diff < 60 * 60 * 2)
        pretty = "${(diff / 3600.0).round()} ${_getTranslation(4, language)}";
      else
        pretty = "${(diff / 3600.0).round()} ${_getTranslation(5, language)}";
    } else if (diff < 60 * 60 * 24 * 30) {
      //Days
      if (diff < 60 * 60 * 24 * 2)
        pretty =
            "${(diff / (3600.0 * 24)).round()} ${_getTranslation(6, language)}";
      else
        pretty =
            "${(diff / (3600.0 * 24)).round()} ${_getTranslation(7, language)}";
    } else if (diff < 60 * 60 * 24 * 365) {
      //Months
      if (diff < 60 * 60 * 24 * 60)
        pretty =
            "${(diff / (3600.0 * 24 * 30)).round()} ${_getTranslation(8, language)}";
      else
        pretty =
            "${(diff / (3600.0 * 24 * 30)).round()} ${_getTranslation(9, language)}";
    } else {
